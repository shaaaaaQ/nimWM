...
proc initXWIndowInfo(winInfo: var XWindowInfo): XWIndowInfo =
  ...
  discard XGrabKey(winInfo.display, XKeysymToKeycode(winInfo.display,
      XStringToKeysym("F1")), Mod1Mask,
    DefaultRootWindow(winInfo.display), true, GrabModeAsync, GrabModeAsync)
  discard XGrabButton(winInfo.display, 1, Mod1Mask, DefaultRootWindow(
      winInfo.display), true,
    ButtonPressMask, GrabModeAsync, GrabModeAsync, None, None)
  discard XGrabButton(winInfo.display, 3, Mod1Mask, DefaultRootWindow(
      winInfo.display), true,
    ButtonPressMask, GrabModeAsync, GrabModeAsync, None, None)
  ...
  while true:
    ...
    if winInfo.ev.theType == KeyPress:
      ...
      if not bev.subwindow.addr.isNil:
        ...
        discard XGetWindowAttributes(winInfo.display, bev.subwindow,
            winInfo.attr.addr);
        winInfo.start = bev;
      ...
      discard XMoveResizeWindow(winInfo.display, mnev.window,
        ...
        max(1, winInfo.attr.width + (if winInfo.start.button ==
            3: xdiff else: 0)),
        max(1, winInfo.attr.height + (if winInfo.start.button ==
            3: ydiff else: 0)))
